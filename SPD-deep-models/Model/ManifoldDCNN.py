...
from Model.Model_SPDNet.spd import SPDNet


class manifoldDCNN(nn.Module):
    def __init__(self, n_class, input_size, device):
        ...
        self.cls = nn.Sequential(
            #nn.BatchNorm1d(filters3[-1] * input_size * (input_size + 1) // 2),
            nn.Dropout(),
            nn.Linear(filters3[-1] * input_size * (input_size + 1) // 2, n_class)
        )
        self.SPDNet = SPDNet(input_size)
    ...
    def forward(self, x):
        ...
        # x = self.cnn(x)
        # _, n, n = x.shape
        b, s, n = x.shape[0], x.shape[1], x.shape[2]
        x = x.reshape(b, s, n * n, 1)
