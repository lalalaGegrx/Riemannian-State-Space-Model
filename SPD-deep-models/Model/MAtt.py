...
from Model.Model_SPDNet.spd import SPDTransform, SPDTangentSpace, SPDRectified
...
class mAtt_bci(nn.Module):
    def __init__(self, embed_in, embed_out, epochs, classes):
        super().__init__()
        ...
        # riemannian part
        self.att2 = AttentionManifold(embed_in, embed_out)
        self.ract2  = SPDRectified()
        
        # R2E
        self.tangent = SPDTangentSpace(embed_out)
        self.flat = nn.Flatten()
        # fc
        self.linear = nn.Linear(embed_out*(embed_out+1)//2*epochs, classes, bias=True)
