...
from Model.SPDCNN import SPDCNN, SPDCNN_small
from MAtt.train_MAtt import train_op, test_op
from Model.utils import BaseDataset_RSSM, calculate_metrics
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', default='SPDCNN')

    parser.add_argument('--path', default='/data/lalalagegrx/RiemannianRNN/EcoGLibrary/SEED_RSSMlatent/', help='Directory')
    ...
    for sub in range(1, args.subjects + 1):
        ...
        Y1 = Data['Label'].squeeze().astype('int32')
        Y1[Y1 == -1] = 0
        samples_cumsum = Data['samples_cumsum'].squeeze().astype('int32')
        args.latents = X1.shape[0]
        args.units = math.ceil(args.latents / 2)
        #for i in range(args.latents):
        #    X1[:, :, i, i] += 0.01

        # Dataset
        dataset = BaseDataset_RSSM(X1, Y1, samples_cumsum, args)
        ...
        for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
            ...
            if args.model == "SPDCNN":
                model = SPDCNN_small(args.latents, args.T, args.classes).to(device)
    ...
    sio.savemat('/data/lalalagegrx/RiemannianRNN/EcoGLibrary/SEED/Results/SPDCNN/SPDCNN_RSSM_10fold_shuffle_results.mat', {'acc':acc_l, 'mf1': mf1_l})
